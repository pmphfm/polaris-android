...
import static agersant.polaris.features.smart_search.SmartSearchAdapterAlbum.AlbumViewType.DISC_HEADER;
import static agersant.polaris.features.smart_search.SmartSearchAdapterAlbum.AlbumViewType.TRACK;


class SmartSearchAdapterAlbum extends SmartSearchAdapter {
    ...
    SmartSearchAdapterAlbum(API api, PlaybackQueue playbackQueue) {
        super();
        this.api = api;
    ...
    public SmartSearchItemHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == DISC_HEADER.ordinal()) {
            View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.view_smart_search_album_disc, parent, false);
            return new SmartSearchItemHolderAlbumDiscHeader(api, playbackQueue, this, itemView, itemQueueStatusView);
        } else {
            View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.view_smart_search_album_item, parent, false);
            return new SmartSearchItemHolderAlbumTrack(api, playbackQueue, this, itemView, itemQueueStatusView);
        }
    }
    ...
    public void onBindViewHolder(SmartSearchItemHolder holder, int position) {
        ...
        if (holder instanceof SmartSearchItemHolderAlbumTrack) {
            ...
            // Assign disc number
            SmartSearchItemHolderAlbumDiscHeader header = (SmartSearchItemHolderAlbumDiscHeader) holder;
            int index = 0;
            for (int discIndex = 0; discIndex < discSizes.size(); discIndex++) {
                if (position == index) {
